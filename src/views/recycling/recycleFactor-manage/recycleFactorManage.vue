<style lang="less">
@import "./recycleFactorManage.less";
</style>
<template>
  <div class="search">
    <Row>
        ...
        <Card>
          <Row>
            <Form ref="searchForm" :model="searchForm" inline :label-width="100" class="search-form">
              <Form-item label="价格因素名称" prop="name">
                <Input
                  type="text"
                  v-model="searchForm.name"
                  clearable
                  placeholder="请输入价格因素名称"
                  style="width: 200px"
                />
              </Form-item>
              <Form-item label="价格因素描述" prop="description">
                <Input
                  type="text"
                  v-model="searchForm.description"
                  clearable
                  placeholder="请输入价格因素描述"
                  style="width: 200px"
                  />
              </Form-item>
              <Form-item label="价格计算公式" prop="priceFormula">
                <Input
                  type="text"
                  v-model="searchForm.priceFormula"
                  clearable
                  placeholder="请输入价格计算公式"
                  style="width: 200px"
                  />
              </Form-item>
              <span v-if="drop">
          ...
          <Row class="operation">
            <Button @click="add" type="primary" icon="md-add">添加商品价格因素</Button>
    ...
    >
      <Form ref="userForm" :model="userForm" :label-width="100" :rules="userFormValidate">
        <FormItem label="价格因素名称" prop="name">
          <Input v-model="userForm.name" autocomplete="off"/>
        </FormItem>
        <FormItem label="价格因素描述" prop="description">
          <Input v-model="userForm.description" autocomplete="off"/>
        </FormItem>
        <FormItem label="价格因素KEY" prop="priceKey">
          <Input v-model="userForm.priceKey" autocomplete="off" placeholder="必须唯一(例如：lowFloor)"/>
        </FormItem>
        <FormItem label="价格公式" prop="priceFormula">
          <Input v-model="userForm.priceFormula" autocomplete="off" placeholder="必须为公式方式（字母，符号，数字）"/>
        </FormItem>
...
export default {
  ...
  data() {
    const isHave = (rule, value ,callback) =>{
      if(this.modalType===0||(this.modalType===1&&this.oldPriceKey!=this.userForm.priceKey)){
        selectByPriceKey({priceKey:this.userForm.priceKey}).then(res=>{
          if(res.success===true){
            callback(new Error('改价格因素KEY已存在'))
          }else if(res.success===false){
            callback();
          }else{
            callback(new Error('价格因素KEY不能未空'))
          }
        })
      }else{
        callback();
    ...
    const isFormula = (rule, value, callback) =>{
      var reg=/^$|^[a-z_A-Z0-9-\.!@#\$%\\\^&\*\)\(\+=\{\}\[\]\/",'<>~\·`\?:;|]+$/;
      if(!reg.test(value)) {
        callback(new Error("公式格式不正确，应如x+(y*5)"));
      }else{
        callback();
      }
    };
    ...
    return {
      ...
      userFormValidate: {
        name: [
          { required: true, message: "价格因素不能为空", trigger: "blur" }
        ],
        description: [
          {required: true, message: "价格因素描述不能为空", trigger: "blur"}
        ],
        priceFormula:[
          {required:false, trigger: "blur"},
          {validator:isFormula,trigger: "blur"}
        ],
        status: [
        ...
        priceKey:[
          {required: true,message: "价格因素KEY不能为空",trigger: "blur"},
            ...
            return h("div", [
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.oldPriceKey = params.row.priceKey;
  ...
  methods: {
    ...
    getUserList() {
      ...
      // }
      getFactorList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success === true) {
      ...
      this.$refs.userForm.validate(valid => {
        if (valid) {
          if (this.modalType === 0) {
            ...
            addFactor(this.userForm).then(res => {
            ...
            // 编辑
            this.submitLoading = true;
            editFactor(this.userForm).then(res => {
              this.submitLoading = false;
              if (res.success === true) {
                this.$Message.success("操作成功");
                this.getUserList();
    ...
    enable(v) {
      this.$Modal.confirm({
        title: "确认启用",
        content: "您确认要启用该价格因素吗 " + v.name + " ?",
        onOk: () => {
          this.operationLoading = true;
          enableFactor(v.id).then(res => {
    ...
    disable(v) {
      this.$Modal.confirm({
        title: "确认禁用",
        content: "您确认要禁用该价格因素吗 " + v.name + " ?",
        onOk: () => {
          this.operationLoading = true;
          disableFactor(v.id).then(res => {
            this.operationLoading = false;
            if (res.success === true) {
              this.$Message.success("操作成功");
              this.getUserList();
    ...
    remove(v) {
      this.$Modal.confirm({
        title: "确认删除",
        content: "您确认要删除该价格因素吗 " + v.name + " ?",
        onOk: () => {
          this.operationLoading = true;
          deleteFactor(v.id).then(res => {
            this.operationLoading = false;
            if (res.success === true) {
