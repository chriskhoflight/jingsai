...
<script>
import { getIntegralRechargeDetailsList } from "@/api/integral";
export default {
  ...
  methods: {
    ...
    getIntegraDetailslListData() {
      this.loading = true;
      getIntegralRechargeDetailsList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
  ...
  watch: {
    "$route.name"(toC, fromC) {
      if (fromC === "integral-manage" && toC === "rechargeDetails") {
